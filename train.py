...
def create_model(architecture="vgg16"):
    ...
    if type(architecture) == type(None):
        ...
        exec("model.name = {}".format(architecture))
...
def train_model(Model, train_data, test_data, Device,
    ...
    for e in range(Epochs):
        ...
        for ii, (inputs, labels) in enumerate(train_data):
            ...
            if steps % print_every == 0:
                ...
                with torch.no_grad():
                    valid_loss, accuracy = validation(model, validate_data, criterion,device)

                print("Epoch: {}/{} | ".format(e + 1, Epochs),
                      "Training Loss: {:.4f} | ".format(running_loss / print_every),
                      "Validation Loss: {:.4f} | ".format(valid_loss / len(test_data)),
                      "Validation Accuracy: {:.4f}".format(accuracy / len(test_data)))
...
if __name__ == '__main__':
    ...
    training_dataset = test_transformer(train_dir)
    validating_dataset = train_transformer(valid_dir)
    testing_dataset = train_transformer(test_dir)

    train_data = data_loader(training_dataset)
    validate_data = data_loader(validating_dataset, train=False)
    test_data = data_loader(testing_dataset, train=False)
    ...
    trained_model = train_model(model, train_data, validate_data,
    ...
    validate_model(trained_model, test_data, device)
